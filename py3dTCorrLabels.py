...
masked_data = apply_mask(nibabel.load(data_file), mask_file_data)
...
def get_corr(tups):
    return [pearsonr(tup[0], tup[1]) for tup in tups]
...
result_p = numpy.zeros((label_values.shape[1], total_count), dtype=numpy.float32)
...
for lab_idx in range(label_values.shape[1]):
    ...
    for i in range(batch_count):
        ...
        corr_struct_orig = pool.map(get_corr, generator)
        corr_struct_ = [val[0] for sublist in corr_struct_orig for val in sublist]
        p_struct_ = [val[1] for sublist in corr_struct_orig for val in sublist]
        result[lab_idx, result_index:result_index+len(corr_struct_)] = corr_struct_
        result_p[lab_idx, result_index:result_index + len(p_struct_)] = p_struct_
...
h5f.create_dataset('p', data=result_p)
h5f.close()
...
for idx in range(result_p.shape[0]):
    dat = result_p[idx, :]
    new_array = numpy.zeros(reshape_size, dtype=numpy.float32)
    new_array[fixed_mask_mat==1] = dat
    vn = nibabel.Nifti1Image(new_array, mask_file_data.affine, mask_file_data.header)
    vn.to_filename(out_ + '_' + str(idx) + 'p.nii')
